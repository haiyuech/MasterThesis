...
All seven dynamics models are integrated into a unified control framework for evaluation. This chapter describes the sampling-based MPC controller employed for trajectory optimization (Section~\ref{sec:mppi}), the iterative learning framework adopted for closed-loop evaluation (Section~\ref{sec:sitlmpc}), and the evaluation objective that links model quality to control performance (Section~\ref{sec:eval-objective}).
...
\end{equation}
where the stage cost $c$ penalizes deviation from a reference path and excessive control magnitude, and $g$ encodes safety constraints such as track boundaries. The constraint penalty weight $\lambda$ controls the trade-off between optimality and feasibility. The final control is computed as a cost-weighted average:
\begin{equation}
...
Given the learned model $\hat{f}$ and a model-based controller $\pi$, the control-level objective is to minimize cumulative cost while enforcing safety:
\begin{equation}
  \min_{\pi,\, \hat{f}} \;\mathbb{E}\!\left[\sum_{t=0}^{T} c(s_t, a_t)\right]
  \label{eq:eval-objective}
\end{equation}
subject to state constraints $s_t \in \mathcal{X}$, input constraints $a_t \in \mathcal{U}$, and safety constraints $g(s_t, a_t) \le 0$, where $c$ is the stage cost and $T$ is the task horizon. Both the MPPI and SIT-LMPC formulations instantiate this objective: MPPI approximates it via sampling over a finite horizon, and SIT-LMPC extends it across iterations with terminal safety guarantees.

Evaluation proceeds in two stages. First, open-loop prediction accuracy is measured on held-out trajectories to assess how well each model captures terrain-dependent dynamics. Second, closed-loop control performance is evaluated by deploying each model within the MPPI and SIT-LMPC controllers, measuring lap time, tracking error, and constraint satisfaction. This two-stage protocol separates predictive fidelity from downstream control utility, because a model that achieves low prediction error does not necessarily yield effective closed-loop behavior.
