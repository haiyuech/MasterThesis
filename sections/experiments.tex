...
\subsection{Platform}
All experiments are conducted on a 1/5th-scale autonomous off-road race car. The vehicle is equipped with a Fixposition Vision-RTK 2 GNSS system for robust localization in an outdoor grassy environment. A GoPro camera mounted on the vehicle captures onboard RGB images at $854 \times 480$ resolution and approximately 30\,Hz for visual feature extraction. The vehicle operates on a closed-loop track in an off-road grassy environment, in which natural terrain variations introduce surface variability that affects tire-road interaction and vehicle dynamics.
